...
write_csv(df_baseline, "data_after_pre_process_for_weights_60_75.csv")


##### drafts #######
df_raw <- df_read 
df <- df_raw %>% filter(id %in% df_baseline$id) %>% mutate(no_data_for_outcome_age = !(id %in% data_for_outcome_age) )

df %>% 
  arrange(id, ageyr, no_data_for_outcome_age) %>% 
  distinct(id, .keep_all = T) %>% 
  ggplot(aes(x = ageyr, group = no_data_for_outcome_age, color = no_data_for_outcome_age)) + 
  geom_histogram() + 
  facet_wrap(~ no_data_for_outcome_age) + 
  xlab("minimal age available")

df %>% 
  arrange(id, desc(ageyr), no_data_for_outcome_age) %>% 
  distinct(id, .keep_all = T) %>% 
  ggplot(aes(x = ageyr, group = no_data_for_outcome_age, color = no_data_for_outcome_age)) + 
  geom_histogram() + 
  facet_wrap(~ no_data_for_outcome_age) + 
  xlab("maximum age available")


df_baseline <- df_baseline %>% filter(!(id %in% data_for_outcome_age) )
df_raw %>% 
  filter(id %in% df_baseline$id) %>% 
  arrange(id, desc(irt_amit)) %>% 
  distinct(id, .keep_all = T) -> lost
...
lost %>%
  pull(irt_amit)/12 + 1900 -> i 
sum(i < 2012 )

# df_raw %>% 
#    filter(!(id %in% df_baseline$id)) %>% 
#    filter(!is.na(dtdx)) %>%
#    mutate(closest_row_to_diag = abs(dtdx-irt_amit)) %>% 
#    arrange(id, closest_row_to_diag) %>% 
#    select(id, irt_amit,dtdx,closest_row_to_diag,interval) %>% 
#    distinct(id, .keep_all = T) %>% 
#    pull(interval) %>% 
#    hist()
...
df_raw <- df_raw %>% mutate(no_baseline = !(id %in% df_baseline$id)) 
df_raw <- df_raw %>% 
  arrange(id, ageyr) %>% 
  distinct(id, .keep_all = T) 
ggplot(df_raw) + aes(x = ageyr, color = no_baseline) + geom_histogram() + facet_wrap(~no_baseline) + xlab("minimal age avaliable")

df_raw %>% filter(no_baseline == TRUE & ageyr > 50)

df_read %>% filter(id == 103509) 
df_baseline %>% filter(have_outcome_age_data == TRUE) %>% pull(id) -> ids 
df_raw %>% filter(id %in% ids) %>% filter(ageyr >= 75) %>% arrange(id, ageyr) %>% distinct(id, .keep_all = T) %>% pull(ageyr) -> ages
summary(ages)
hist(ages)

df_baseline$have_outcome_age_data %>% table()

df_raw %>% filter(id %in% df_baseline$id) %>% arrange(id, desc(ageyr)) %>% distinct(id, .keep_all = T) %>% pull(ageyr) %>% hist()



no_data_after_outcome_age <- df_raw %>% 
  filter(id %in% df_baseline$id) %>% 
  arrange(id, desc(ageyr)) %>% distinct(id, .keep_all = T) %>% 
  filter(ageyr < outcome_age+1 & crc == 0) %>% 
  pull(id)
...
# who don't have data at outcome_age+ 
df_baseline <- df_baseline %>% mutate(have_outcome_age_data = !(id %in% no_data_after_outcome_age)) 
stats <- rbind(stats, stats_func(df_baseline, "not infected & no data for outcome_age"))
stats



df_baseline %>% filter(crc == 1 & is.na(msin)) %>% pull(id) -> a 

df_baseline %>% filter(crc == 1 & is.na(msin)) %>% pull(id) -> c


setdiff(c, b)

df_raw %>% select(id, interval, ageyr, crc, dt_crc, msin) %>% filter(id == 50918900)

outcome <- df_raw %>% 
  filter(id %in% df_baseline$id) %>% 
  mutate(ageyr_distance_from_outcome = abs(ageyr - outcome_age)) %>% 
  filter(ageyr_distance_from_outcome <= 2 | agedx <= (outcome_age + 2) ) %>% 
  arrange(id, ageyr_distance_from_outcome) %>% 
  distinct(id, .keep_all = T) %>% 
  select(id, dt_crc) %>% 
  rename(crc = dt_crc) %>% 
  mutate(has_outcome_data = TRUE)

df_baseline <- df_baseline %>% 
  select(-crc) %>% 
  left_join(outcome, by = "id") %>% 
  mutate(has_outcome_data = ifelse(is.na(has_outcome_data), FALSE, TRUE))

baseline_vec <- c(60)
outcome_age_vec <- c(75) 
all_stats <- list()
k <- 1 
for (baseline in baseline_vec) {
  for (outcome_age in outcome_age_vec) {
    all_stats[[k]]  <- clean_data(df_read, baseline, outcome_age)
    names(all_stats)[k] <- str_c("baseline=", baseline, " , outcome=", outcome_age)
    k <- k+1 
  }
}
