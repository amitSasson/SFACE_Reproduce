library(tidyverse)
df <- read_csv("/udd/nhasn/data_after_pre_process_for_weights_60_70.csv") #109149
...
df <- df %>% filter(!is.na(packyr)) #105717
...
df <- df %>% 
  ...
  filter(!(crc == 1 & is.na(msin))) #filter after weights #104933
...
df <- df %>% 
  ...
  filter(outcome_age_data == TRUE) #66257

sum_weights <- sum(df$weight)
df <- df %>% mutate(relative_weight=weight/sum_weights)

write_csv(df, "data_after_weights_for_analysis_has_outcome_data_60_70.csv")
