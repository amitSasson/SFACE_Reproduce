                                   ...
                                   A_par = A_par)
{
  ...
  po_probs <- function(U = U, X = X, A = A, y1_par, y2_par)
  {
    X_y <- X
    X_y[,2] <- 2^(X_y[,2])
    e.y1 <- exp(y1_par$gamma0 + y1_par$gammaU*U + y1_par$gammaA*A + c(y1_par$gammaX%*%(t(X_y))))
    e.y2 <- exp(y2_par$gamma0 + y2_par$gammaU*U + y2_par$gammaA*A + c(y2_par$gammaX%*%(t(X_y))))
  ...
  #y_A1_probs[y_A1_probs < 0] <- 0 #TODO: asset all rows sum to 1 and positive
...
stand_total_naive <- function(df, sface)
{
  ...
  n <- nrow(df)
  return(sum(pred_treat[,self]) / sum(pred_untr[,self]))
}
...
po_sface_func <- function(df, sface)
{
  if(sface == "sface1")
  {
    ...
    return(mean(df$y1_A1)/mean(df$y1_A0))
  }

  if(sface == "sface2")
  {
    df <- subset(df, y1_A0 == 0 & y1_A1 == 0)
    return(mean(df$y2_A1)/mean(df$y2_A0))
  }
}
...
stand_cond_naive <- function(df, sface)
{
  if (sface == "sface1")
  {
    ...
    return(sum(pred_treat) / sum(pred_untr))
  }
  if (sface == "sface2")
  {
...
save(ans, file = str_c("sim_breakY_RR_", as.character(args),"_", Sys.time(), ".RData"))
